...
Example judge_even_3 :
    test_ceval (t_update empty_state X 3) judge_even
    = Some (3, 0, 1).
Proof. reflexivity. Qed.
(** [] *)
...
(* I'm not good at informal proof. However, the 
   proposition [ceval_step__ceval] proves is quite obvious in that
   the program terminates in limited steps. *)
[]
*)
...
Proof.
    induction n.
    - intros. inversion H.
    - intros. inversion H. destruct m.
        + simpl. reflexivity.
        + simpl. simpl in H. apply IHn in H. apply H.
Qed.
...
Proof.
  ...
  - inversion IHHce1. inversion IHHce2.
    remember (x <=? x0) as b. destruct b.
    + exists (S x0). simpl. 
      assert (ceval_step st c1 x0 = Some st'). {
        apply (ceval_step_more x x0 st st').
        - assert ((x <=? x0) = true). { rewrite Heqb. reflexivity. }
          rewrite leb_iff in H1. apply H1.
        - apply H.
      }
      rewrite H1. apply H0.
    + exists (S x). simpl.
      rewrite H.
      apply (ceval_step_more x0 x st' st'').
      -- assert ((x <=? x0) = false). { rewrite Heqb. reflexivity. } 
         apply leb_false in H1. apply leb_iff in H1. apply H1.
      -- apply H0.
  ...
  - inversion IHHce. exists (S x).
    simpl. rewrite H. apply H0.
  - exists 1. simpl. rewrite H. reflexivity.
  - inversion IHHce1. inversion IHHce2. remember (x <=? x0) as b1.
    destruct b1.
    + exists (S x0). simpl. rewrite H.
      assert (ceval_step st c x0 = Some st'). {
          apply (ceval_step_more x x0 st st').
          - assert ((x <=? x0) = true). { rewrite Heqb1. reflexivity. }
            rewrite leb_iff in H2. apply H2.
          - apply H0.
      }
      rewrite H2. apply H1.
    + exists (S x). simpl. rewrite H.
      rewrite H0. 
      apply (ceval_step_more x0 x st' st'').
      -- assert ((x <=? x0) = false). { rewrite Heqb1. reflexivity. }
         apply leb_false in H2. apply leb_iff in H2. apply H2.
      -- apply H1.
Qed.
